...
public class ObjectVolume {
	...
	// calc method
	//
	// Determines if the normal to the triangle formed from the last three
	...
	//
	// Calculates the area of the triangle formed from the last three points
	//
	// Calulates the volume of a tetrahedron formed from the four input
	// points.
	//
	// Calculates the center of volume of the tetrahedron and multiplies these
	...
	double[] calc(double x1, double y1, double z1, double x2, double y2,
		...
		double d21, d31, d32, d41, d42, d43;
		double d12, d13, d14, d23, d24, d34;
		double A = 0, B = 0, C = 0, D = 0;

		double[] ans = { 0, 0, 0, 0, 0 };
		double dot = 0;
		// Make vector a (from points 2 and 3) and vector b (from points 2 and
		...
		// When the normal vector of the triangle is 0 or close to zero the
		// volume of the tetrahedron is zero or close to zero and is ignored.
		// The NaN
		// value is caused by having a negative value under the square
		// root in the volume formula.
		if (dot == 0) {
			return (ans);
		}
		...
		// input points
		d21 = (x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1)
				+ (z2 - z1) * (z2 - z1);
		d31 = (x3 - x1) * (x3 - x1) + (y3 - y1) * (y3 - y1)
				+ (z3 - z1) * (z3 - z1);
		d32 = (x3 - x2) * (x3 - x2) + (y3 - y2) * (y3 - y2)
				+ (z3 - z2) * (z3 - z2);
		d41 = (x4 - x1) * (x4 - x1) + (y4 - y1) * (y4 - y1)
				+ (z4 - z1) * (z4 - z1);
		d42 = (x4 - x2) * (x4 - x2) + (y4 - y2) * (y4 - y2)
				+ (z4 - z2) * (z4 - z2);
		d43 = (x4 - x3) * (x4 - x3) + (y4 - y3) * (y4 - y3)
				+ (z4 - z3) * (z4 - z3);
		...
		// Calculate the four components of the determinant in Tatagilia's
		// formula. This is a five by five matrix but since the first term is
		// zero only four (4 x 4) determinants need to be determined.
		A = -d23 * d34 * d42 - d24 * d32 * d43 - d12 * d34 * d43 + d12 * d23
				* d34 + d12 * d24 * d43 + d13 * d34 * d42 - d13 * d24 * d42
				+ d13 * d24 * d32 + d14 * d32 * d43 + d14 * d23 * d42 - d14
				* d23 * d32;
		B = -d21 * d34 * d43 + d23 * d34 * d41 + d24 * d31 * d43 - d13 * d34
				* d41 + d13 * d21 * d34 + d13 * d24 * d41 - d13 * d24 * d31
				- d14 * d31 * d43 + d14 * d21 * d43 - d14 * d23 * d41 + d14
				* d23 * d31;
		C = d21 * d34 * d42 - d24 * d31 * d42 + d24 * d32 * d41 + d12 * d34
				* d41 - d12 * d21 * d34 - d12 * d24 * d41 + d12 * d24 * d31
				+ d14 * d31 * d42 - d14 * d32 * d41 - d14 * d21 * d42 + d14
				* d21 * d32;
		D = d21 * d32 * d43 + d23 * d31 * d42 - d23 * d32 * d41 + d12 * d31
				* d43 - d12 * d21 * d43 + d12 * d23 * d41 - d12 * d23 * d31
				- d13 * d31 * d42 + d13 * d32 * d41 + d13 * d21 * d42 - d13
				* d21 * d32;

		double vol = Math.sqrt((A + B + C + D) / 288);
