...
public class ObjectVolume {
	...
	double[] calc(double x1, double y1, double z1, double x2, double y2,
		...
		// When the normal vector is 0 or close to zero the volume of the
		// tetrahedron is zero or close to zero and is ignored. The NaN
		...
		// Use the four input points of the tetrahedron to determine its six
		// edge lengths. The three lowercase lengths (u, v and w) are defined as
		// opposite edges to its corresponding uppercase value (U, V, W).
		double u = Math.sqrt(((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1))
				+ ((z2 - z1) * (z2 - z1)));
		double v = Math.sqrt(((x3 - x1) * (x3 - x1)) + ((y3 - y1) * (y3 - y1))
				+ ((z3 - z1) * (z3 - z1)));
		double w = Math.sqrt(((x4 - x1) * (x4 - x1)) + ((y4 - y1) * (y4 - y1))
				+ ((z4 - z1) * (z4 - z1)));
		double U = Math.sqrt(((x4 - x3) * (x4 - x3)) + ((y4 - y3) * (y4 - y3))
				+ ((z4 - z3) * (z4 - z3)));
		double V = Math.sqrt(((x4 - x2) * (x4 - x2)) + ((y4 - y2) * (y4 - y2))
				+ ((z4 - z2) * (z4 - z2)));
		double W = Math.sqrt(((x3 - x2) * (x3 - x2)) + ((y3 - y2) * (y3 - y2))
				+ ((z3 - z2) * (z3 - z2)));

		// Calculate the Volume of the tetrahdron using the six line lengths
		// from above.
		double up = (v * v) + (w * w) - (U * U);
		double vp = (w * w) + (u * u) - (V * V);
		double wp = (u * u) + (v * v) - (W * W);

		double vol = (Math.sqrt((4 * (u * u) * (v * v) * (w * w))
				- ((u * u) * (up * up)) - ((v * v) * (vp * vp))
				- ((w * w) * (wp * wp)) + (up * vp * wp))) / 12;
